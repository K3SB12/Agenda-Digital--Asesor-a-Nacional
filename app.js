// ===== SISTEMA DE GESTIÓN DE TAREAS DRTE =====
class AgendaDRTE {
    constructor() {
        this.tasks = this.loadFromStorage('drte_tasks') || [];
        this.templates = this.loadFromStorage('drte_templates') || [];
        this.evidences = this.loadFromStorage('drte_evidences') || [];
        this.settings = this.loadFromStorage('drte_settings') || {
            theme: 'dark',
            notifications: true,
            autoBackup: false
        };
    ...
    saveToStorage(key, data) {
        try {
            ...
            this.showNotification('Error guardando datos. Espacio de almacenamiento posiblemente lleno.', 'error');
            return false;
        }
    }
    ...
    openTaskModal(type) {
        ...
        // Preseleccionar categoría según tipo
    ...
    renderTasks(tasksToRender = null) {
        ...
        // Aplicar filtro actual si no se especifican tareas
    ...
    saveAsTemplate() {
        ...
        if (!formData.title || !formData.category) {
            this.showNotification('Complete los campos obligatorios', 'warning');
            return;
        }
        ...
        this.saveToStorage('drte_templates', this.templates);
    ...
    loadTemplate(templateId) {
        ...
        this.selectLocation(template.location || 'remote');
        
        this.showNotification('Plantilla cargada', 'success');
    }
...
function initializeApp() {
    ...
    setTimeout(() => {
        agenda.showNotification('Agenda DRTE cargada exitosamente', 'success');
    }, 1000);
}
...
function backupToDrive() {
    agenda.showNotification('Función de Backup a Google Drive (requiere configuración OAuth)', 'info');
}

function downloadAllFiles() {
    agenda.showNotification('Descargando todos los archivos...', 'info');
    // Implementar descarga múltiple
}
